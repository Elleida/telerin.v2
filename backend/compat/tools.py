...
def call_llm_prompt(prompt: str, backend: str = "ollama", model: str | None = None, timeout: int = 60) -> str:
    ...
    if backend in ("gemini", "google"):
        model_to_use = model or os.getenv("GEMINI_MODEL") or "gemini-3-flash-preview"
    else:
...
def classify_query_intent(query: str) -> str:
    ...
    Usa el backend LLM activo (_CURRENT_LLM_BACKEND).
    """
    if not query:
        return "data_search"
...
Responde solo con una palabra: GREETING, SYSTEM_INFO o DATA_SEARCH"""
    ...
    try:
        classification = call_llm_prompt(prompt, backend=backend, model=model, timeout=30).strip().upper()
        if "GREETING" in classification:
            print(f"   🤖 Query clasificada como: GREETING (saludo/cortenía) [{backend}]")
            return "greeting"
        elif "SYSTEM_INFO" in classification:
            print(f"   🤖 Query clasificada como: SYSTEM_INFO (pregunta sobre el sistema) [{backend}]")
            return "system_info"
        else:
            print(f"   🤖 Query clasificada como: DATA_SEARCH (búsqueda en BD) [{backend}]")
            return "data_search"
    except Exception as e:
...
def _is_generic_date_query(search_query: str) -> bool:
    ...
    backend, model = get_active_llm_backend()
    try:
        classification = call_llm_prompt(classification_prompt, backend=backend, model=model, timeout=20).strip().upper()
        print(f"   [_is_generic_date_query] Respuesta LLM [{backend}]: '{classification[:100]}'")
...
def hybrid_search(
    ...
    reranking_time = 0.0
    print(f"   🔍 DEBUG RERANKING: VIVO_CLIENT={'activo' if VIVO_CLIENT else 'NONE'}, resultados={len(all_results)}")
    ...
    # Ordenar: si se rerankeron usar rerank_score, si no usar relevance_score original
    if reranking_time > 0:
        all_results.sort(key=lambda x: x.get("rerank_score", x.get("relevance_score", 0)), reverse=True)
    else:
        all_results.sort(key=lambda x: x.get("relevance_score", 0), reverse=True)
...
def custom_sql_search(sql_query: str, search_text: str = "") -> str:
    ...
    try:
        if VIVO_CLIENT and formatted_results:
            ...
            # Ordenar por rerank_score (semántico) en lugar de relevance_score (CrateDB BM25)
            formatted_results.sort(key=lambda x: x.get('rerank_score', x.get('relevance_score', 0)), reverse=True)
    except Exception as e:
...
def generate_response_internal(
    ...
    # Aplica reranking solo si los resultados no fueron ya rerankeados
    # (hybrid_search y custom_sql_search ya rerankean internamente; image_text_search no)
    already_reranked = any(
        isinstance(r, dict) and r.get('rerank_score') is not None
