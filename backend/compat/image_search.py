...
from backend.compat.tools import _rerank_results, VIVO_CLIENT, VIVOEMBCLIENT_AVAILABLE
...
def get_image_embedding(image_data: bytes) -> Optional[List[float]]:
    ...
    if not VIVOEMBCLIENT_AVAILABLE or VIVO_CLIENT is None:
        print("VIVOClient no está disponible. Instala vivoembclient para usar búsqueda de imágenes.")
        return None
    ...
    try:
        ...
        if not result:
            print("Respuesta vacía de VIVOClient")
            return None
        ...
        if not isinstance(first_result, dict):
            print(f"Resultado no es un diccionario: {type(first_result)}")
            return None
        ...
        if embedding:
            ...
            print("No se encontró campo 'embedding' en la respuesta")
            return None

    except Exception as e:
        print(f"Error al procesar imagen con VIVOClient: {str(e)}")
        import traceback
        traceback.print_exc()
        return None
...
def search_similar_images(embedding: List[float], limit: int = 60, rerank_query: Optional[str] = None) -> Tuple[List[dict], Optional[str]]:
    ...
    try:
        ...
        png_base_url = os.getenv("PNG_BASE_URL", PNG_BASE_URL)
        ...
        if response.status_code == 200:
            ...
            try:
                if rerank_query and _rerank_results:
                    print("   🔄 Rerankeando resultados de imagen...")
            ...
            print(f"Error en consulta a CrateDB: {response.text}")
            return [], sql_query

    except Exception as e:
        print(f"Error buscando imágenes similares: {str(e)}")
        return [], None
