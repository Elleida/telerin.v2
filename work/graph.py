...
from config import OLLAMA_BASE_URL, OLLAMA_LLM_MODEL, CONVERSATION_MEMORY_CONFIG
from tools import (
    SEARCH_TOOLS,
...
def create_search_agent() -> ChatOllama:
    llm = ChatOllama(
...
def search_node(state: GraphState) -> GraphState:
    ...
    if conversation_memory is not None and CONVERSATION_MEMORY_CONFIG.get("auto_enhance_query", True):
        print("\n🧠 Analizando contexto conversacional...")
        try:
            if handler:
            ...
            pass
        try:
            is_contextual_follow_up = conversation_memory.is_contextual_follow_up(user_query)
            ...
            print(f"   Entidades globales: {conversation_context.get('global_entities', {})}")
        except Exception:
            enhanced_query = None
            is_contextual_follow_up = False
            conversation_context = None
    ...
    print(f"🔍 Query para búsqueda: {actual_query}")
